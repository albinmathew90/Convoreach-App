...
export class BaileysAdapter implements IWhatsAppEngine {
  ...
  async sendTextMessage(chatId: string, text: string, mentions?: string[], options?: { quotedMessageId?: string }): Promise<MessageResult> {
    this.ensureReady();
    const quoted = options?.quotedMessageId ? await this.requireStored(options.quotedMessageId) : undefined;
  ...
  async sendImageMessage(chatId: string, media: MediaInput, options?: { quotedMessageId?: string }): Promise<MessageResult> {
    this.ensureReady();
  ...
  async sendVideoMessage(chatId: string, media: MediaInput, options?: { quotedMessageId?: string }): Promise<MessageResult> {
    this.ensureReady();
    const { data, mimetype } = await this.resolveMediaBuffer(media);
    const quoted = options?.quotedMessageId ? await this.requireStored(options.quotedMessageId) : undefined;
    return this.sendContent(chatId, {
  ...
  async sendAudioMessage(chatId: string, media: MediaInput, options?: { quotedMessageId?: string }): Promise<MessageResult> {
    this.ensureReady();
    const { data, mimetype } = await this.resolveMediaBuffer(media);
    const quoted = options?.quotedMessageId ? await this.requireStored(options.quotedMessageId) : undefined;
    return this.sendContent(chatId, { audio: data, mimetype, ptt: media.ptt ?? false }, { quoted });
  }

  async sendDocumentMessage(chatId: string, media: MediaInput, options?: { quotedMessageId?: string }): Promise<MessageResult> {
    this.ensureReady();
    const { data, mimetype } = await this.resolveMediaBuffer(media);
    const quoted = options?.quotedMessageId ? await this.requireStored(options.quotedMessageId) : undefined;
    return this.sendContent(chatId, {
  ...
  private async processInboundMessage(msg: WAMessage, opts?: { skipMedia?: boolean }): Promise<void> {
    try {
      ...
      // Attempt to resolve unknown LIDs from quoted messages (e.g. template button replies)
      // so that the reply is correctly associated with the contact's existing phone number chat.
      const senderJid = msg.key.participant ?? remoteJid;
      if (senderJid.endsWith('@lid') && !this.sessionStore.resolvePhone(senderJid)) {
        try {
          let contextInfo: any;
          if (normalizedRoot && typeof normalizedRoot === 'object') {
            for (const val of Object.values(normalizedRoot)) {
              if (val && typeof val === 'object' && 'contextInfo' in val) {
                contextInfo = (val as any).contextInfo;
                break;
              }
            }
          }
          if (contextInfo?.stanzaId && this.config.messageStore) {
            const originalMsg = await this.config.messageStore.getMessage(this.config.dbSessionId, contextInfo.stanzaId);
            const originalRemoteJid = originalMsg?.key?.remoteJid;
            if (originalRemoteJid && originalRemoteJid.endsWith('@s.whatsapp.net')) {
              this.sessionStore.addLidMappings([{ lid: senderJid, pn: originalRemoteJid }]);
              this.logger.debug('Resolved unknown LID from quoted message', { lid: senderJid, pn: originalRemoteJid, stanzaId: contextInfo.stanzaId });
            }
          }
        } catch (err) {
          this.logger.warn('Failed to resolve LID from quoted message', { lid: senderJid, error: err instanceof Error ? err.message : String(err) });
        }
      }
